...
- Per-assignee weekly task files (`output/exec-comms/weekly-tasks/{YYYY-WW}/{owner}.md`) — generated alongside the digest; ready to email
...
- `state/okrs.md`
- **Cyvore GTM Weekly Sync Google Sheet** — **only when `audience=weekly`**. Read via `python3 tools/sheets/read-weekly-sync.py [--week YYYY-WW] [--prior]` (the tool reads the Sheet via service-account auth — no file uploads, no `state/` files). Tab named `DD.MM.YYYY`. Stage labels in this Sheet are the canonical pipeline taxonomy. Sheet ID lives in `tools/sheets/google-sheets-config.json`. Replaces the deprecated `state/weekly-customer-sync.xlsx` (archived during the 2026-W22 migration).

### Reads on demand (by exact path)

- Latest `output/pipeline/{YYYY-WW}/snapshot.md` — for any digest, board, investor, all-hands artifact
...
- `output/exec-comms/weekly-tasks/{YYYY-WW}/{owner-slug}.md` — per-assignee task files generated by the exec-comms skill (audience=weekly) via `python3 tools/sheets/read-weekly-sync.py --by-assignee`
...
- **Weekly digest** / "what happened this week" → run exec-comms with `audience=weekly`. Requires (a) a current-week tab in the Cyvore GTM Weekly Sync Sheet (auto-created Mondays 06:00 by `tools/sheets/generate-weekly-tab.py`; create on demand if missing), (b) fresh `output/pipeline/{YYYY-WW}/snapshot.md`, (c) fresh `state/weekly-context.md`. Generates the digest **and** per-assignee task files.
