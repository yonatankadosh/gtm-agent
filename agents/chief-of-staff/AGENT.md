...
- `output/exec-comms/weekly-tasks/{YYYY-WW}/{owner-slug}.md` — per-assignee task files generated by the exec-comms skill (audience=weekly) via `python3 tools/read-weekly-sync.py --by-assignee`
...
- **Weekly digest** / "what happened this week" → run exec-comms with `audience=weekly`. Requires (a) a current-week tab in the Cyvore GTM Weekly Sync Sheet (auto-created Mondays 06:00 by `tools/generate-weekly-tab.py`; create on demand if missing), (b) fresh `output/pipeline/{YYYY-WW}.md`, (c) fresh `state/weekly-context.md`. Generates the digest **and** per-assignee task files.
