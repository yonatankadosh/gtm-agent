...
- Pipeline snapshot — `output/pipeline/{YYYY-WW}/snapshot.md`
...
To send each owner their list: `python tools/email/send-email.py --to "{email}" --subject "Weekly tasks — Week {YYYY-WW}" --file "output/exec-comms/weekly-tasks/{YYYY-WW}/{owner}.md"`
...
Defaults: recipient = `default_recipient` from `tools/email/email-config.json`; subject = the digest's H1. Override with `--to`, `--cc`, `--subject`, `--attach`, `--no-xlsx`, `--no-logo`. Always `--dry-run` first if you've changed the structure.

---

*Generated by Chief of Staff (exec-comms). Sources: Cyvore GTM Weekly Sync Sheet (tab `{DD.MM.YYYY}`), `output/pipeline/{YYYY-WW}/snapshot.md`, `output/pipeline/{YYYY-WW}/sync-log.md`, `state/weekly-context.md`.*
