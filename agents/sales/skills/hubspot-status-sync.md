...
2. **Monday morning (or any time you want to consolidate):** run `tools/sheets/sheet-hubspot-merge.py` — the outer-join consolidator. It does a 3-way merge between (a) the Sheet at generation time = the snapshot in `state/sheet-snapshots/{tab}.json`, (b) the current Sheet, and (c) current HubSpot. It writes a plan + report and applies the plan via `tools/hubspot/hubspot-batch.py`.
3. **Monday morning, after step 2:** run `tools/sheets/generate-weekly-tab.py` — generates the new week's tab from now-current HubSpot. Closed Lost / Disqualified records are filtered out. Tier / Assignee / moving-status are carried forward from the prior tab. A fresh snapshot is written to seed step 2 for next week.

**Outer-join discipline:** the consolidator never deletes from HubSpot. Removing a row from the Sheet is interpreted as "I don't want this on the meeting view this week," not "kill it." Hard kills still go through the explicit kill flow (`tools/hubspot/hubspot-batch.py kill` or the Phase 4 batch path below).
...
| **EXCLUDED_STAGE** | Sheet stage is `Closed Lost` / `Disqualified`. | Skipped. Use `tools/hubspot/hubspot-batch.py kill` (or the Phase 4 batch path below) for explicit kills with audit-trail notes. |
...
- The HubSpot MCP is unavailable / errored — this skill's REST path (via `tools/hubspot/hubspot-batch.py`) is the canonical recovery path.
...
| HubSpot MCP errored / disconnected | this skill — REST fallback via `tools/hubspot/hubspot-batch.py` |
...
|---|---|---|
| Update deal/lead Status + Next Step | `manage_crm_objects.updateRequest` (deals) / `tools/hubspot/hubspot-leads.py update\|batch` (leads) | `tools/hubspot/hubspot-batch.py update --type {deal\|lead} --id N --prop K=V` |
| Move stage / kill | `manage_crm_objects.updateRequest` with new dealstage / hs_pipeline_stage | `tools/hubspot/hubspot-batch.py {move\|kill}` |
| Create new deal | `manage_crm_objects.createRequest` | `tools/hubspot/hubspot-batch.py create-deal --name "..." --stage in-meetings` |
| Create new lead (with primary Company association) | `manage_crm_objects.createRequest` with associations block | `tools/hubspot/hubspot-batch.py create-lead --name "..." --stage connected` (auto-resolves company) |
| Log a Note | `manage_crm_objects.createRequest` with `objectType: "notes"` | `tools/hubspot/hubspot-batch.py note --type {deal\|lead} --id N --body "..."` |
| Mixed batch | One `manage_crm_objects` call per object type | `tools/hubspot/hubspot-batch.py batch --plan plan.json` |

**Always check MCP availability first.** If the HubSpot MCP `STATUS.md` shows "errored" / "Not connected", fall back to `tools/hubspot/hubspot-batch.py` immediately — do NOT spend time debugging MCP during a live meeting. The REST tool uses the same Private App token (`tools/hubspot/hubspot-config.json`), runs through the same audit-log discipline, and produces the same HubSpot record state.
...
- **Cyvore GTM Weekly Sync Google Sheet** — the user-curated weekly tabs (one tab per `DD.MM.YYYY`). Configured at `tools/sheets/google-sheets-config.json` (sheet_id + service-account credentials). Read via `python3 tools/sheets/read-weekly-sync.py`. Replaces the deprecated `state/weekly-customer-sync.xlsx` archived under `state/archive/` after the 2026-W22 migration.
- **Sheet snapshot** at `state/sheet-snapshots/{tab-name}.json` — the post-generation BASE for the 3-way merge. Written automatically by `tools/sheets/generate-weekly-tab.py` on every successful run. If missing, the consolidator stops and asks you to regenerate the tab; fallback is the legacy manual flow.
...
For the FULL set of operations (updates + kills + creates + notes), the Private App at `tools/hubspot/hubspot-config.json` must have ALL of these scopes:
...
Earlier versions of this skill listed only the first two. The full list above is required for create flows. If a scope is missing, `tools/hubspot/hubspot-batch.py` will surface a 403 with `MISSING_SCOPES` and the exact scope name — add it in HubSpot Settings → Integrations → Private Apps → your app → Auth tab → Save → re-run. No restart of Cursor needed for REST flows.

### Association typeIds (used by create + note flows)

Hard-coded in `tools/hubspot/hubspot-batch.py`. Documented here so they're visible to the skill:
...
2. Save the token at `tools/hubspot/hubspot-config.json` (gitignored — same pattern as `tools/email/email-config.json`):
...
3. Create **two** custom properties on the Lead object via the HubSpot UI (Settings → Properties → Lead → Create property):
   ...
   These must be created in the UI because the Private App lacks `crm.schemas.*.write` scopes (HubSpot doesn't expose a Leads-schema scope to Private Apps as of 2026-05). `tools/hubspot/hubspot-leads.py init-properties` is provided as a fallback for when/if HubSpot expands the scope set — it's idempotent.
...
```bash
python3 tools/sheets/generate-weekly-tab.py
```
...
Use this only when the consolidator doesn't apply (e.g. no snapshot exists yet because this is the first run after the migration). The 4-phase manual categorization below produces the same plan shape as the consolidator.

Run these steps in order. At every step, ask "**so what?**" — don't list facts without commercial interpretation.

### Step 0: Confirm the sync sheet tab exists

If the Mon 06:00 `generate-weekly-tab.py` cron has not fired yet (the team is meeting earlier than usual), the user can run it manually from chat: `python3 tools/sheets/generate-weekly-tab.py` — that materializes today's tab in the Sheet from current HubSpot state. This is purely additive; it never overwrites a tab the team has already edited (the `--force` flag is opt-in).

### Step 1: Read the latest Sheet tab

Run `python3 tools/sheets/read-weekly-sync.py` (no flags) — emits the latest tab as JSON to stdout. The user can also pass `--week YYYY-WW` to pin a specific week.
...
**Leads (via REST, Phase 2 only):** run `python3 tools/hubspot/hubspot-leads.py list` — emits all Leads as JSON with `id`, `hs_lead_name`, `hs_pipeline_stage`, `cyvore_next_step`, `cyvore_weekly_status`.
...
- Decide deal vs. lead by stage:
  - Lead pipeline (`New / Attempting / Connected / Qualified`) → create Lead via `tools/hubspot/hubspot-batch.py create-lead --name "..." --stage {stage}` (auto-resolves primary Company; passes `--company-domain` if known).
  - Deal pipeline (`In meetings/conversations` and beyond) → create Deal via `tools/hubspot/hubspot-batch.py create-deal --name "..." --stage {stage}`.
...
- Most common: a Disqualified lead that came back to life after a meeting. Use `tools/hubspot/hubspot-batch.py move --type lead --id N --to connected` (with optional `--prop cyvore_weekly_status=...`).
...
- Apply via `tools/hubspot/hubspot-batch.py kill --type {deal|lead} --id N --reason "..."` — this moves stage AND logs a Note in one call.
...
**Preferred path: one batch plan via `tools/hubspot/hubspot-batch.py`.** Materialize the categorized phases (step 4) as a JSON plan and apply in one call:

```bash
python3 tools/hubspot/hubspot-batch.py batch --plan /tmp/sync-plan-2026-WNN.json
```
...
**MCP path (when available):** the `tools/hubspot/hubspot-batch.py` operations map 1:1 onto `manage_crm_objects.updateRequest` / `createRequest` calls. If the user prefers the MCP confirmation table, swap the tool calls and keep the same plan structure — the audit log section below is identical either way.
...
- [ ] After the consolidator applied, `tools/sheets/generate-weekly-tab.py` was run to materialize the next week's tab + a fresh snapshot
...
tools/sheets/sheet-hubspot-merge.py                  →  the consolidator implementation (read+plan; --apply to push).
tools/sheets/generate-weekly-tab.py                  →  REVERSE direction. HubSpot → Sheet. Run after the consolidator on Monday.
...
2. **Monday morning, before the team meeting:** run `tools/sheets/sheet-hubspot-merge.py` (dry-run). Show the user the report. After confirmation, re-run with `--apply`.
3. **Monday morning, after the consolidator:** run `tools/sheets/generate-weekly-tab.py`. New tab + fresh snapshot are written. The team reads this tab during the meeting.
