...
**5b. Enrich contacts (agent executes — not the user):** The agent must run Apollo in the terminal for this account's domain. For each key contact with a known full name, run `python3 tools/apollo/apollo-enrich.py enrich --domain [domain] --name "[Full Name]"` and add the returned work email to the Key People table. If only a title is known (e.g., "CISO"), run `python3 tools/apollo/apollo-enrich.py search --domain [domain] --title "[title]"` first (free), pick the best match, then run `enrich` for that person. Do not ask the user to run Apollo manually. If the tool fails (missing API key, no match), record "needs verification" in the table and list it under Research Gaps.
